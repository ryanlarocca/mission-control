...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let q = sb
      .from("campaign_sends")
      .select(
        "id, touch_number, subject, body, status, edited, variant, prompt_version, approved_at, scheduled_for, sent_at, error, created_at, contact:campaign_contacts (id, name, email, import_flags, property_address)"
      )
